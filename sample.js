...
function getRemainingLetters() {
    ...
    wordDisplay.querySelectorAll('.letter').forEach(letterElement => {
        if (!letterElement.innerText.trim()) {
            remainingLetters.push(letterElement.dataset.letter.toLowerCase());
        }
    });
...
function revealVowel() {
    ...
    let remainingLetters = getRemainingLetters(); // Function to get unrevealed letters in the current word
